# Place Preference simsalRbim data preparation
# 27.10.2022
...
#Spalten hinzuf?gen (mouse no., group, experiment (Structural, active, housing, ENR1, ENR2, group, preference enr 1))
data = t(data)
...
#data = mutate( data, group = c(rep("ENR1", 120), rep("ENR2", 120), rep("ENR3", 120)) )#f?gt Spalte mit Gruppe hinzu

#f?r simsalRbim ben?tigte Spalten filtern (subjectID, optionA, optionB, quantityA, quantityB)
...
### inactive times ###################################################################################
#read/input data (Linkes Enrichment)
data = read.delim("./place_preference_30daysL_inactive time.txt", header= T, sep = "\t", dec= ".")
...
#erstelltes dataframe speichern
write.table(data, "MOPPS data prepared for simsalRbim_inactive time.txt", sep = "\t", row.names = T)


### active times binary #################################################################################
#data preparation mit binaren ergebnis 0/1 f?r simsalRbim
